...
### 5.1 실용적 케이스
- [x] 와이어 두께 1mm, 롤 내경 50mm일 때 10m 길이의 회전량을 계산한다
- [x] 와이어 두께 2mm, 롤 내경 100mm일 때 1080도 회전의 길이를 계산한다
- [x] 와이어 두께 0.5mm, 롤 내경 25mm일 때 역변환을 검증한다
- [x] 롤 내경 변경 후 같은 길이에 대해 다른 회전량이 계산되는지 확인한다

### 5.2 극단적 케이스
- [x] 매우 얇은 와이어(0.1mm)에서 정확하게 동작한다
- [x] 매우 두꺼운 와이어(10mm)에서 정확하게 동작한다
- [x] 작은 롤(10mm 내경)에서 정확하게 동작한다
- [x] 큰 롤(500mm 내경)에서 정확하게 동작한다
- [x] 와이어 두께가 롤 내경보다 클 때 정확하게 동작한다
...
### 6.1 정밀도 테스트
- [x] 부동소수점 연산 오차가 허용 범위(1e-6) 내에 있다
- [x] 반복 계산 시 오차가 누적되지 않는다

### 6.2 성능 테스트
- [x] 단일 계산이 1ms 이내에 완료된다
- [x] 1000회 연속 계산이 100ms 이내에 완료된다
...
### 7.1 기본 빌드 설정
- [x] CMakeLists.txt로 프로젝트를 빌드할 수 있다
- [x] 라이브러리 타겟이 생성된다
- [x] 테스트 실행 파일이 생성된다
...
### 8.1 코드 문서화
- [x] 모든 public 메서드에 주석이 있다
- [x] 클래스에 설명 주석이 있다
- [x] 파라미터와 반환값이 문서화되어 있다

### 8.2 예제 코드
- [x] 기본 사용 예제 코드가 작성되어 있다
- [x] 롤 내경 설정 예제가 포함되어 있다
- [x] 내경 변경에 따른 계산 차이를 보여주는 예제가 있다
- [x] 예제 코드가 컴파일되고 실행된다
